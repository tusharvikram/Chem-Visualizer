...
<head>
  ...
  <!-- Inlined rather than linked so the generated file keeps its icon when opened
       straight from disk, with no extra request. -->
  <link rel="icon" href="data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%2064%2064'%3E%3Cdefs%3E%3ClinearGradient%20id='m'%20x1='0'%20y1='0'%20x2='1'%20y2='1'%3E%3Cstop%20offset='0'%20stop-color='%236366f1'/%3E%3Cstop%20offset='1'%20stop-color='%23a855f7'/%3E%3C/linearGradient%3E%3C/defs%3E%3Cg%20stroke='url(%23m)'%20stroke-width='6.5'%20stroke-linecap='round'%3E%3Cline%20x1='32'%20y1='32'%20x2='13'%20y2='48'/%3E%3Cline%20x1='32'%20y1='32'%20x2='51'%20y2='48'/%3E%3C/g%3E%3Ccircle%20cx='13'%20cy='48'%20r='7.5'%20fill='url(%23m)'/%3E%3Ccircle%20cx='51'%20cy='48'%20r='7.5'%20fill='url(%23m)'/%3E%3Ccircle%20cx='32'%20cy='32'%20r='13'%20fill='url(%23m)'/%3E%3Ccircle%20cx='25.2'%20cy='12.5'%20r='4.1'%20fill='%23ec4899'/%3E%3Ccircle%20cx='38.8'%20cy='12.5'%20r='4.1'%20fill='%23ec4899'/%3E%3C/svg%3E" />
  ...
  <style>
    :root {
      /* Contrast ratios against the card surface are noted per token. Projected
         slides wash light colours out, so body text targets 7:1 rather than the
         4.5:1 WCAG AA minimum. The previous glass palette measured 3.8-4.3:1. */
      --primary: #4338ca;      /* 7.9:1  interactive + the molecular geometry value */
      ...
      /* Pink belongs to lone pairs and nothing else. It carries meaning in the
         viewer and in the legend, so it is never used as decoration. */
      --lone-pair: #ec4899;
    }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .dashboard {
      ...
      /* Solid, not frosted: the translucent stack was pulling every foreground
         colour toward the background and breaking contrast. */
      background: var(--surface);
      border: 1px solid var(--border-strong); border-radius: 20px;
      box-shadow: 0 12px 32px -12px rgba(18, 22, 28, 0.18); overflow: hidden;
    }
    .sidebar {
    ...
    label { font-size: 0.85rem; font-weight: 600; color: var(--ink-soft); text-transform: uppercase; letter-spacing: 0.05em; }
    select {
      width: 100%; padding: 12px; border-radius: 10px; border: 1px solid #d6dae2;
      background: #fff; color: var(--ink);
      font-family: inherit; font-size: 1rem; cursor: pointer; transition: all 0.15s;
    }
    select:hover { border-color: var(--primary); }
    select:focus-visible { outline: 2px solid var(--primary); outline-offset: 1px; }
    .toggles { display: flex; gap: 10px; }
    .btn {
      flex: 1; padding: 10px; border: 1px solid var(--border-strong); border-radius: 10px; background: #fff;
      font-weight: 600; color: var(--primary); cursor: pointer;
      font-family: inherit; transition: all 0.15s;
    }
    .btn:hover { border-color: var(--primary); background: #f7f8fc; }
    .btn.active { background: var(--primary); color: #fff; border-color: var(--primary); }
    .btn.active:hover { background: #3730a3; }
    .btn:focus-visible { outline: 2px solid var(--primary); outline-offset: 1px; }
    .info-card {
      background: var(--surface-sunken); border-radius: 12px; padding: 12px 14px; border: 1px solid var(--border);
    }
    ...
    .badge-polar { background: #dbe4ff; color: #2b3a8f; }
    .badge-nonpolar { background: #e8eaee; color: #33383f; }
    .fact-box {
      margin-top: auto; background: var(--surface-sunken);
      border-left: 3px solid var(--primary); padding: 10px 12px; border-radius: 0 8px 8px 0;
      font-size: 0.85rem; font-style: italic; line-height: 1.4; color: var(--ink-soft);
    }
    .card-divider {
      height: 1px; background: var(--border); margin: 9px 0;
    }
    .credit {
      font-size: 0.78rem; color: var(--ink-muted); text-align: center;
      padding-top: 2px; letter-spacing: 0.01em;
    }
    /* Laptops at high display scaling leave a short viewport — a 16" 2880x1800
       panel at 200% has only ~630px once browser chrome is subtracted. Compact
       the panel there rather than letting it scroll or clip. */
    @media (max-height: 760px) {
      .sidebar { gap: 8px; padding: 14px 20px; }
      h1 { font-size: 1.35rem; }
      .subtitle { font-size: 0.78rem; margin-top: 2px; }
      .info-card { padding: 10px 12px; }
      .info-row { font-size: 0.84rem; margin-bottom: 4px; }
      .btn { padding: 8px; font-size: 0.88rem; }
      select { padding: 9px; font-size: 0.92rem; }
      .fact-box { padding: 9px 10px; font-size: 0.78rem; }
      .card-divider { margin: 7px 0; }
    }
    .credit a {
      color: var(--primary); font-weight: 600; text-decoration: none;
      border-bottom: 1px solid rgba(67, 56, 202, 0.4); transition: all 0.15s;
    }
    .credit a:hover { border-bottom-color: var(--primary); }
    .viewer-area {
      position: relative; background: #fff; margin: 10px; border-radius: 14px;
    ...
    .legend {
      ...
      padding: 9px 13px; border-radius: 10px; font-size: 0.8rem; color: var(--ink);
    ...
    .angle-note { margin-top: 8px; font-size: 0.7rem; color: var(--ink-muted); line-height: 1.35; font-style: italic; }
    .dipole-note { font-size: 0.72rem; color: var(--primary); line-height: 1.35; }
